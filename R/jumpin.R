...
#' total_downloads("alm")
#'}
total_downloads <- function(pkg, start = as.Date("2012-10-01") , today = NULL) {
    ...
    if (data$status != 404) 
        {
            ...
            cran_downloads <- ifelse(cran_return == 200, total_downloads(package, start, today), 0)
            # --------------------------------------------------------
...
#'}
generate_html <- function(out, browse = TRUE) {
last_generated <- now("UTC")
message("writing out html \n")
html <- whisker::whisker.render(readLines("template2.html"))
